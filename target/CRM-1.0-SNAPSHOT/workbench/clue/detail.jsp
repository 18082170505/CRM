	...
	$(function(){
		...
		$("#searchActivity").keydown(function (event) {
			if (event.keyCode==13){
				...
				//必须要return false,否则按完回车键会默认清空页面所有内容
				return false
			}
		})
		
		//为绑定按钮添加事件，绑定选中的市场活动信息
		$("#bundBtn").click(function () {
			//获取选中的市场活动信息记录对应的Jquery对象
			...
			$.ajax({
				url:"bund.do",
				//可以直接传递拼接的字符串参数(如cId=xxx&aId=xxx&aId=xxx)
				data:param,
				type:"post",
				dataType:"json",
				success:function (data) {
					...
					if (data.success){
						...
						//刷新关联的市场活动信息列表
						showActivityList()
						//清空搜索框中的值，把全选按钮中的勾去掉并清空查询到的市场活动信息
...
<body>
	...
	<div style="position: relative; left: 40px; top: -30px;">
		...
		<div style="position: relative; height: 50px; width: 500px;  top: -72px; left: 700px;">
			<button type="button" class="btn btn-default" onclick="window.location.href='convert.jsp?id=${c.id}&fullname=${c.fullname}&appellation=${c.appellation}&owner=${c.owner}&company=${c.company}';"><span class="glyphicon glyphicon-retweet"></span> 转换</button>
