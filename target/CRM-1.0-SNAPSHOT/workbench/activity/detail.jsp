...
<script type="text/javascript">
        ...
        $("#saveRemarkBtn").click(function () {
            $.ajax({
                ...
                success:function (data) {
                    /*
                    data:{"success":true/false,"ar":{备注信息}}
                     */
                    //将textarea文本域中的内容清空
                    $("#remark").val("")
                    if (data.success){
                        //在textarea文本域上方新增一条备注信息
                        var html=""
                        ...
                        html += '&nbsp;&nbsp;&nbsp;&nbsp;';
                        //注意给动态标签添加监听事件时，如果有参数，括号内的参数要加引号
        ...
        $("#updateRemarkBtn").click(function () {
            ...
            $.ajax({
                ...
                success:function (data) {
                     ...
                     */
                    if (data.success){
                        //更新成功后修改对应备注的div中的noteContent、editTime和editBy
		...
		$.ajax({
			...
			success:function (data){
				...
				$.each(data,function (i,n) {
				     //技巧：以循环方式拼接表中每条记录时，在循环中以每条记录的id值给标签id赋值，因为记录的id是每条记录的唯一标识
                     //所以也可以标识用于拼接每个记录的标签
        ...
        if (confirm("确定删除此备注吗？")){
            $.ajax({
                ...
                success:function (data){
                    ...
                    if (data.success){
                        //刷新备注信息
                        //此做法行不通，因为使用的是before方法进行拼接，每次删除后都会将删除后的备注信息拼接到后面
                        //showRemarkList()

                        //应该找到要删除的备注对应的div,将该div移除即可
                        $("#"+id).remove()
                    }else {
                        alert("删除备注信息失败！")
                    }
    ...
    function editRemark(id){
	    //将id赋予到隐藏域对象中，便于后续的更新操作
        $("#remarkId").val(id)
	    //获取h5标签对应id的备注内容
        var noteContent = $("#e"+id).html()
        //将备注内容赋予到模态窗口对应的textarea文本域中
