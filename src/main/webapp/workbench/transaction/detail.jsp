...
<script type="text/javascript">
	...
	function showHistoryList(){
		$.ajax({
			url:"getTranHistoryListByTranId.do",
			data:{"tranId":"${t.id}"},
			type:"get",
			dataType:"json",
	...
	function changeStage(stage,i){
		$.ajax({
			url: "changeStage.do",
			data: {
				//后台需要根据tranId修改交易信息，知道修改之后的stage,以及每次修改后新增一条交易历史（money和expectedDate）
				"id":"${t.id}",
				"stage":stage,
				"money":"${t.money}",
			...
			success:function (data) {
				/*
				  data:{"success":true/false,"t":{修改后的交易信息}}
				  后面会用到t中的stage、possibility、editBy和editTime来更新详细信息页
				 */
				if (data.success){
					//更新详细信息中的阶段、可能性、修改人和修改时间
	...
	function changeIcon(stage,index1){
		...
		//当前阶段可能性(注意不能从request域中取，因为request域中存放的是更新之前的可能性)
		var currentPossibility = $("#possibility").html()
		//当前阶段的下标
		var index = index1
		//前面正常阶段和后面丢失阶段的分界点下标（这里是7）
		//注意只有java的八大基本数据类型和String类型可以转换为js中的数据类型，要加引号
		var point = "<%=point%>"
		...
		//如果当前阶段可能性为0,前7个一定是黑圈，后两个一个是红叉一个是黑叉
